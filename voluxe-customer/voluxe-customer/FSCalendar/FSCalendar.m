...
- (void)setCollectionViewScrollEnabled:(BOOL)enabled {
    _collectionView.scrollEnabled = enabled;
}
...
- (void)scrollToDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    NSInteger sectionOffset = [self.calculator indexPathForDate:date atMonthPosition:FSCalendarMonthPositionCurrent].section;
    NSInteger dayOffset = [self.calculator indexPathForDate:date atMonthPosition:FSCalendarMonthPositionCurrent].row;

    if (!self.floatingMode) {
        switch (_collectionViewLayout.scrollDirection) {
            case UICollectionViewScrollDirectionVertical: {
                [_collectionView setContentOffset:CGPointMake(0, sectionOffset * _collectionView.fs_height) animated:animated];
                break;
            }
            case UICollectionViewScrollDirectionHorizontal: {
                [_collectionView setContentOffset:CGPointMake(sectionOffset * _collectionView.fs_width, 0) animated:animated];
                break;
            }
        }
    ...
    /*
    if (!animated) {
        self.calendarHeaderView.scrollOffset = sectionOffset;
    }
     */
}
